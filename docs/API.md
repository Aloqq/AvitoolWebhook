...
События с одинаковой тройкой **`event` + `message` + `account`** (пустой `account` считается как отсутствующий) не отправляются в Telegram повторно, если с момента **успешной** последней отправки такого же ключа прошло меньше заданного окна (по умолчанию **60 секунд**). Настройки: `webhook.deduplication-enabled` (по умолчанию `true`) и `webhook.deduplication-window-seconds`. Запись в `webhook.log` при этом создаётся **на каждый** допустимый запрос; при пропуске из-за дедупликации `telegram_sent` будет `false`, `telegram_error` — `null`.
