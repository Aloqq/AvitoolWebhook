...
После успешной проверки авторизации и тела запроса сервер **немедленно** отвечает `200 OK`, а обработка (опциональная дедупликация, вызов Telegram API, запись строки в `logs/webhook.log`) выполняется **асинхронно**, чтобы не блокировать поток обработки HTTP.
...
По умолчанию **дедупликация выключена** — каждый допустимый POST обрабатывается как новое событие для Telegram. Если включить `webhook.deduplication-enabled: true`, события с одинаковой тройкой **`event` + `message` + `account`** (пустой `account` считается как отсутствующий) не отправляются в Telegram повторно, пока с момента **успешной** последней отправки такого же ключа не истечёт окно `webhook.deduplication-window-seconds` (по умолчанию **60**). Запись в `webhook.log` создаётся **на каждый** допустимый запрос; при пропуске из-за дедупликации `telegram_sent` будет `false`, `telegram_error` — `null`.
