...
Минималистичный **Spring Boot 3** сервис на **Java 21**: приём **HTTP webhook** (JSON), проверка **Bearer-токена**, асинхронная отправка уведомлений в **Telegram** (MarkdownV2, fenced-блок с языком `log`), опциональная дедупликация в Telegram, построчный JSON-лог в файл.
...
| Markdown в Telegram **400** | Экранирование MarkdownV2 для fenced-блока с языком `log`; при новых символах в payload смотрите ответ API в логе. |

---

## Лицензия

Проект распространяется под **[MIT](LICENSE)**: можно **свободно использовать, менять и распространять** (в том числе в коммерческих продуктах) — достаточно сохранять текст лицензии и уведомление об авторских правах в копиях. Автор не против любого добросовестного использования.
